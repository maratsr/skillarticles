...
class MarkdownCodeView private constructor(
    ...
    init {
        tv_codeView = MarkdownTextView(context, fontSize*0.85f).apply {
            ...
            isFocusable = true // Чтобы внутри могла захватывать фокус
            isFocusableInTouchMode = true
        }
        sv_scroll = object : HorizontalScrollView(context){
            override fun getTopFadingEdgeStrength(): Float {
                return 0f // 0 - без fade, 1 - с fade
            }
        }.apply {
            overScrollMode = View.OVER_SCROLL_NEVER
            isHorizontalFadingEdgeEnabled = true // Тень на правой части
            scrollBarSize = scrollBarHeight
            setFadingEdgeLength(fadingOffset) // длина fading граней (затуманивание текста на краях)
            addView(tv_codeView) // добавить контент с текстом кода
        }
        addView(sv_scroll)
